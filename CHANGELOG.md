...
- iface
    - Pin the one-element hardware address cache to the instant it was resolved at, and drop it whenever the neighbor cache is filled. A cache hit returns before `neighbor_cache.lookup()`, which is the only thing enforcing the 60 s `ENTRY_LIFETIME`, so a resolved address was previously handed out indefinitely: past the entry's expiry, and past a peer moving to another hardware address. Making the timestamp part of the key means the entry cannot survive a re-stamp of `now` by any path, and within a single instant only a `fill` or `flush` can change what a lookup would return. This was caught by the `netsim` snapshot, which had been silently drifting since the cache was introduced — the test could not compile, so nothing ran it.
    ...
    - Evict a TCP 4-tuple from the index as soon as the socket stops owning it, so the common teardowns need no `forget_tcp_socket` call. A server written the documented way — `if !socket.is_open() { socket.listen(port) }` — never removes the socket from the `SocketSet`, so nothing called `forget_tcp_socket` and the closed connection's entry stayed until a segment carrying that exact 4-tuple arrived, which for an ephemeral peer port never happens. The table filled after half its capacity in connection *lifetimes* and every later connection fell back to the linear scan for good. The test is tuple ownership rather than a terminal state, since an RST in SYN-RECEIVED returns a listening socket to `Listen` rather than `Closed`; `TimeWait` is evicted too, because the linear scan can re-ACK a retransmitted FIN and an index slot held for 2MSL is exactly the occupancy the table cannot afford.
- tcp
    - Count duplicate ACKs per segment rather than once per batch, so fast retransmit still fires when a peer's duplicates arrive together in one RX burst. `process_batch_finalize` ran the detection once for the whole batch, and that single evaluation took the not-a-duplicate branch and merely recorded `local_rx_last_ack` — so three duplicates in a burst counted as zero, where sequentially they count as two. Fast retransmit arms at exactly the third, and a peer sends its duplicates back to back, so on the batch path (including `process_batch_zero_copy`, the DPDK ingress path) loss recovery silently fell back to an RTO timeout: hundreds of milliseconds instead of about one RTT. The window-update test the detection depends on is now also evaluated per segment, matching `process()`, where the second of two segments carrying the same window is not a window update.
    - Report `PollAt::Now` from `Socket::poll_at` when the interface no longer has the socket's local address. `dispatch_setup` is the only place that resets such a socket, and the egress fast filter skips `dispatch` entirely for a socket reporting `PollAt::Ingress`, so an otherwise idle connection was never reset and kept its state on an address the interface had dropped. `poll_at` was written to mirror the start of `dispatch()` as a scheduling hint, where being early is harmless; using it as a skip condition makes the mirroring a correctness requirement, which is now documented on the function.
    ...
    - Bound the advertised receive window by free zero-copy descriptor slots. Byte-denominated window accounting alone let a peer put more segments in flight than the descriptor array can hold; the excess was dropped without an ACK and the retransmission resent the whole window, overflowing again. Only sockets with a registered release callback are bounded, so sockets driven through `process()` in a zero-copy build are unaffected. The gate is release rather than retain because descriptors are stored regardless of the retain callback, which is optional for transports whose frames already outlive the ingress batch.
    - Accept data the peer sent under a previously advertised, wider receive window. Byte accounting keeps the advertised right edge monotone, but the zero-copy slot bound does not — every segment costs a whole slot however few bytes it carried — so a run of small segments retracted the edge below what the peer had already been cleared to send. Segments in that gap were treated as out-of-window and answered with a challenge ACK, rate-limited to one per second. The advertisement still shrinks, as RFC 1122 4.2.2.16 permits; only acceptance now tracks the furthest edge ever advertised.
    - Reconcile an incoming zero-copy segment against the descriptors already held, trimming it or superseding them so descriptors never overlap. The payload was trimmed against the window, whose left edge is the contiguous frontier, but not against out-of-order descriptors past it, so a retransmission overlapping one — go-back-N, a spurious RTO resending from `snd_una`, or a peer that recombined its send queue — was stored twice. `recv_zero_copy` then subtracted the bytes it had consumed from a smaller `stream_offset`: a panic in debug, and in release a wrapped offset no later segment could match, leaving the socket unable to deliver anything again and its frames never released. Overlaps are located by binary search over the sorted array and superseded descriptors are evicted as one contiguous run, so the reconciliation adds no per-segment scan to the in-order arrival path — which matters most exactly when the array is deep, behind an application that is not draining.
    - Saturate the adjusted last-advertised window at zero. Accepting data past the retracted right edge lets the ACK frontier move beyond that edge, and the remainder `last_scaled_window` computes from it then went negative — `TcpSeqNumber - TcpSeqNumber` panics on underflow in every build, so any later `poll_at` or `dispatch` brought the stack down. Zero is also the right answer: none of the last advertisement is left, so a window update is due.
    - Reserve one zero-copy descriptor slot for a segment covering the window's left edge. Out-of-order segments could otherwise fill the array while the left edge was missing — the ordinary single-loss case — collapsing the advertised window to zero; the peer's retransmission of the missing segment was then refused by the zero-window check, so the gap was never filled, no bytes ever became contiguous, `recv_zero_copy` could free nothing, and the connection stalled permanently.
...
- Rust Edition bumped to 2024. ([#1084](https://github.com/smoltcp-rs/smoltcp/pull/1084))
- `heapless` dependency bumped to v0.9. ([#1083](https://github.com/smoltcp-rs/smoltcp/pull/1083))
- iface
    - IPv6 SLAAC (Stateless Address Autoconfiguration), gated behind a new `slaac` feature flag. ([#1039](https://github.com/smoltcp-rs/smoltcp/pull/1039))
    - Add `slaac_updated_at` method to check when SLAAC addresses were last updated. ([#1130](https://github.com/smoltcp-rs/smoltcp/pull/1130))
    - Add getters for the default IPv4/IPv6 route and `is_ipv4_gateway`/`is_ipv6_gateway` helpers. ([#1129](https://github.com/smoltcp-rs/smoltcp/pull/1129))
    - Exhaust egress socket state on `poll`, so sockets don't have to wait for the next poll to send queued data. ([#1059](https://github.com/smoltcp-rs/smoltcp/pull/1059))
    - Remove always-Ok `Result`s in `consume()`. ([#1093](https://github.com/smoltcp-rs/smoltcp/pull/1093))
    - Honor `any_ip` when checking local address. ([#1119](https://github.com/smoltcp-rs/smoltcp/pull/1119))
    - Add `iface_max_route_count-0` feature flag to disable the routing table entirely. ([#1057](https://github.com/smoltcp-rs/smoltcp/pull/1057))
    - Improve IPv4 source address selection for multi-subnet interfaces. ([#1074](https://github.com/smoltcp-rs/smoltcp/pull/1074))
    - Fix `poll_at` returning stale timestamps due to `silent_until` expiry check. ([#1127](https://github.com/smoltcp-rs/smoltcp/pull/1127))
    - Fix fragment payload sizes not being a multiple of eight octets. ([#1116](https://github.com/smoltcp-rs/smoltcp/pull/1116))
    - Fix incorrect packet length after defragmentation. ([#1094](https://github.com/smoltcp-rs/smoltcp/pull/1094))
    - Fix compilation of IPv6+Multicast without Ethernet. ([#1043](https://github.com/smoltcp-rs/smoltcp/pull/1043))
    - Log and drop IPv6 packets requiring fragmentation (IPv6 routers must not fragment). ([#1038](https://github.com/smoltcp-rs/smoltcp/pull/1038))
- tcp
    - Add zero window probe support. ([#1026](https://github.com/smoltcp-rs/smoltcp/pull/1026))
    - Close socket if the local IP is no longer assigned to the interface. ([#1113](https://github.com/smoltcp-rs/smoltcp/pull/1113))
    - Reject bytes outside the receive window. ([#1079](https://github.com/smoltcp-rs/smoltcp/pull/1079))
    - Don't accept RST packets on listening sockets. ([#1058](https://github.com/smoltcp-rs/smoltcp/pull/1058))
    - Don't send TCP RST when packet is handled by a raw socket. ([#1069](https://github.com/smoltcp-rs/smoltcp/pull/1069))
    - Send challenge ACK for duplicate ACK in LAST-ACK state. ([#1126](https://github.com/smoltcp-rs/smoltcp/pull/1126))
    - Fix retransmit exponential backoff, align to RFC 6298. ([#1023](https://github.com/smoltcp-rs/smoltcp/pull/1023))
    - Restart retransmit timer on new data ACK. ([#1018](https://github.com/smoltcp-rs/smoltcp/pull/1018))
    - Fix FIN retransmit in CLOSING state. ([#1026](https://github.com/smoltcp-rs/smoltcp/pull/1026))
    - Add `pause_synack` feature flag to allow user code to withhold SYN|ACK. ([#1063](https://github.com/smoltcp-rs/smoltcp/pull/1063))
- socket
    - ICMP: support binding to a TCP port. ([#1089](https://github.com/smoltcp-rs/smoltcp/pull/1089))
    - ICMP: add `no-auto-icmp-echo-reply` feature flag to disable automatic echo replies. ([#1106](https://github.com/smoltcp-rs/smoltcp/pull/1106))
    - Raw: allow receiving all protocols and IP versions (unfiltered mode). ([#1067](https://github.com/smoltcp-rs/smoltcp/pull/1067))
    - Raw: fix panic when payload buffer exceeds packet size during fragmentation. ([#1077](https://github.com/smoltcp-rs/smoltcp/pull/1077))
    - DHCPv4: reuse DHCPOFFER transaction ID in DHCPREQUEST. ([#1061](https://github.com/smoltcp-rs/smoltcp/pull/1061))
    - DHCPv4: fix panic when T1 < T2 < lease duration is not respected by server. ([#1029](https://github.com/smoltcp-rs/smoltcp/pull/1029))
    - DNS: fix compilation when `socket-dns` is enabled but `socket-udp` isn't. ([#1041](https://github.com/smoltcp-rs/smoltcp/pull/1041))
    - Implement `AnySocket` for the `Socket` enum. ([#1092](https://github.com/smoltcp-rs/smoltcp/pull/1092))
- phy
    - `Tracer`: update public API to allow custom inspection and printing of packets. ([#1076](https://github.com/smoltcp-rs/smoltcp/pull/1076))
    - `TunTapInterface`: no longer automatically enables `medium-ethernet` feature. ([#1055](https://github.com/smoltcp-rs/smoltcp/pull/1055))
- wire
    - Add generic IPv4/IPv6 packet parsing support to `IpRepr`. ([#1087](https://github.com/smoltcp-rs/smoltcp/pull/1087))
    - Make `Cidr` initialization `const`. ([#1036](https://github.com/smoltcp-rs/smoltcp/pull/1036))
    - Add conversion from `Endpoint` into `SocketAddr`. ([#1124](https://github.com/smoltcp-rs/smoltcp/pull/1124))
    - Use newly stable IP methods from `core::net`. ([#1115](https://github.com/smoltcp-rs/smoltcp/pull/1115))
    - Fix `RawHardwareAddress` panic when parsing to specific link-layer address. ([#1027](https://github.com/smoltcp-rs/smoltcp/pull/1027))
    - Fix multicast panic when `max_resp_code` is zero. ([#1047](https://github.com/smoltcp-rs/smoltcp/pull/1047))
    - Fix partial checksum in tcpdump/pcap. ([#1015](https://github.com/smoltcp-rs/smoltcp/pull/1015))
