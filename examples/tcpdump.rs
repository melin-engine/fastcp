...
use fastcp::phy::{Device, RawSocket, RxToken};
use fastcp::time::Instant;
use fastcp::wire::{EthernetFrame, PrettyPrinter};
use std::env;
use std::os::unix::io::AsRawFd;

fn main() {
    let ifname = env::args().nth(1).unwrap();
    let mut socket = RawSocket::new(ifname.as_ref(), fastcp::phy::Medium::Ethernet).unwrap();
    loop {
